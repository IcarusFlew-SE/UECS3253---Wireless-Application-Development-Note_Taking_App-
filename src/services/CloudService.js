...
const upsertLocalNote = async note => {
  ...
  await db.executeSql(
    ...
    [
      ...
      note.category_id || 3, note.color || '#BED8FF',
      note.isPinned ? 1 : 0, note.lastUpdated || Date.now(),
      1, note.is_deleted ? 1 : 0, note.deleted_at || null,
      note.is_archived ? 1 : 0, note.archived_at || null,
    ]
  );
};
...
export const CloudSyncService = {
  ...
  const db = await getDbPromise();
  const [result] = await db.executeSql('SELECT * FROM notes WHERE isSynced = 0');
  const notes = [];
  ...
  for (const n of notes) {
    await db.executeSql('UPDATE notes SET isSynced = 1 WHERE id = ?', [n.id]);
  }
  ...
  deleteNote: async (userId, noteId, hardDelete = false) => {
    ...
    if (hardDelete) {
      getDb().execute('DELETE FROM notes WHERE id = ?', [noteId]);
